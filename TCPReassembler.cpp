...
std::vector<const unsigned char*> TCPReassembler::reassembling_packets(std::vector<const unsigned char*> tcp_packets)
{
    ...
    for (unsigned i = 0; i != tcp_packets.size(); i++) {
        const struct ether_header* ethernet_header = (struct ether_header*)tcp_packets.at(i);
        ...
        auto reassembled_packet = (const unsigned char*) malloc(ntohs(ipHeader->ip_len)+sizeof(struct ether_header));
        ...
        while (this->summary_length < this->dns_length && j < tcp_packets.size()) {
            const struct ether_header* ether_header = (struct ether_header*)tcp_packets.at(j);
            u_char* payload = nullptr;
